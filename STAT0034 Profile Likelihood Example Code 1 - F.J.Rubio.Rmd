...
#Defining two equation objects, selectEq and obsEq1, which define the 
...
ssMLE.2step = selection(selectEq, obsEq1, data = MEPS2001, method = "2step")
ssMLE.2step
ssMLE
...
#We use the AIC to perform stepwise variable selection.
inds.to.check.obs = c(1:ncol(dm.obs))
inds.to.check.sel = c(1:ncol(dm.sel1))
best.inds.obs = c()
best.inds.sel = c()
AICs.over.models = rep(NA, length(inds.to.check.obs)+length(inds.to.check.sel))
#Going from zero covariates considered to all covariates, which is represented 
#by the while condition
j = 0
best.models = list(NA)
total.coeff.no = length(inds.to.check.obs) + length(inds.to.check.sel)
while(j < total.coeff.no){  
  #Iterating over every covariate in the two models, using the for loop and the
  #iterable inside.
  AIC = rep(NA, length(c(inds.to.check.obs, inds.to.check.sel)))
  for(i in 1:length(AIC)){
    #Making a vector called AIC to store our AICs. 
    #A dual conditional ifelse in order to evaluate the AIC properly, since we 
    #combined the vectors of observable and selection models into one iterable. 
    if(i <= length(inds.to.check.obs)){
      #Evaluating the AIC. 
      dummy = 2*MLETobit2(init = rep(0, 17), out = y, sel = sel, deso = 
        dm.obs[ ,c(inds.to.check.obs[i], best.inds.obs)], 
        dess = dm.sel1[,best.inds.sel])$OPT$objective +
        2*(length(c(best.inds.obs, best.inds.sel))+1+2)
    }
    else{
      #Evaluating the AIC again directly. 
      dummy = 2*MLETobit2(init = rep(0, 17), out = y, sel = sel, deso = 
        dm.obs[ ,best.inds.obs], 
        dess = dm.sel1[,c(inds.to.check.sel[i-length(inds.to.check.obs)],
                          best.inds.sel)])$OPT$objective +
        2*(length(c(best.inds.obs, best.inds.sel))+1+2)
    }
    AIC[i] = dummy
  }
  #Getting the best of the AICs using the max() command, and storing this in the
  #variable best.AIC
  best.AIC = max(AIC)
  #Getting the index corresponding to the best AIC using the which.max() command
  #and storing this inthe variable best.ind
  best.ind = which.max(AIC)
  AICs.over.models[j] = best.AIC
  #Using an if-else conditional pair to store the best indices for each process.
  if(best.ind <= length(inds.to.check.obs)){
    #Concatenating
    best.inds.obs = c(best.inds.obs, inds.to.check.obs[best.ind])
    inds.to.check.obs = inds.to.check.obs[-best.ind]
  }
  else{
    best.inds.sel = c(best.inds.sel, 
                      inds.to.check.sel[i-length(inds.to.check.obs)])
    inds.to.check.sel = inds.to.check.sel[-(best.ind-length(inds.to.check.obs))]
  }
  best.models = c(best.models, list(best.inds.obs, best.inds.sel))
  j = j+1
}
# #Below is some code taken from the MLETobit2 function, in order to do a study 
# #on the effects of changing the correlation coeficient, rho. 
# #As this was taken from the MLETobit2 function, comments will be more sparing. 
# #In general, the parameters will be replaced by the corresponding parameters, 
# #with ".test" appended onto them, e.g. out -> out.test. The replacements
# #are done using the corresponding MLEs and inputs from example 1, above. 
# #Where there are differences, comments will highlight them. 
# out.test = as.vector(y); sel.test = as.vector(sel)
# x1.test = as.matrix(cbind(1, dm.obs)); w1.test = as.matrix(cbind(1, dm.sel1))
# p.test = ncol(x1.test); q.test = ncol(w1.test)
# #Here, we only apply the link to sigma.test, as we will iterate over rho, and 
# #we evaluate the Tobit-2 log-likelihood within the range [-1, 1]. 
# sigma.test = MLE[1]
# beta.test = MLE[c(3:(p.test+2))]; alpha.test = MLE[(p.test+3):(p.test+q.test+2)]
# #We'll opt to not perform the reparametrisation of beta as our main focus is on
# #rho. 
# x_beta.test = x1.test%*%beta.test; w_alpha.test = w1.test%*%alpha.test
# inds0.test = (sel.test == 0); inds1.test = (sel.test == 1)
# #Define here the number of steps to iterate over, in our case, 500 was deemed 
# #sufficiently granular. 
# no.steps = 500
# #Define the vector loglik.Tobit2 to store our log-likelihoods in.
# loglik.Tobit2 = rep(NA, no.steps)
# #Define an indexing dummy variable, i, to store our log-likelihoods within 
# #loglik.Tobit2
# i = 1
# #Initiate a for loop for rho in [-1, 1], iterating over seq(0,1, 
# #length.out = no.steps).

# #TODO: Check this again.
# for (rho in seq(-1, 1, length.out = no.steps)){
#   #Evaluate the log-likelihood for the particular rho, ceteris paribus at the
#   #MLEs.
#   ll = sum(pnorm(-w_alpha.test[inds0.test], log = T)) +
#        sum(pnorm((w_alpha.test[inds1.test] + rho*(out.test[inds1.test] -
#             x_beta.test[inds1.test])/sigma.test)/sqrt(1 - rho^2), log = T)) +
#        sum(dnorm(out.test[inds1.test], mean = x_beta.test[inds1.test],
#                  sd = sigma.test, log = T))
#   #Returning the log-loss, which is the objective we seek to minimise.
#   loglik.Tobit2[i] = ll
#   #Incrementing the dummy variable by 1.
#   i=i+1
# }
# 
# ll.Tobit2.rho = function(rho){
#   ll = sum(pnorm(-w_alpha.test[inds0.test], log = T)) +
#        sum(pnorm((w_alpha.test[inds1.test] + rho*(out.test[inds1.test] -
#             x_beta.test[inds1.test])/sigma.test)/sqrt(1 - rho^2), log = T)) +
#        sum(dnorm(out.test[inds1.test], mean = x_beta.test[inds1.test],
...
# 
# loglik.Tobit2
# #Plotting our results here.
# #Firstly we compare our results against the optimisation algorithm as a check -
# #we find the standardised absolute distance between the two.
# #Note we add the objective of the optimisation step, OPT,
# #as the objective there is the negative of the log-likelihood.
# abs((max(loglik.Tobit2) + OPT$OPT$objective)/OPT$OPT$objective)
# #Very small difference - seems that
# 
# #Try to do beta/sigma reparametrisation.
# 
# #Relative likelihood
# plot(loglik.Tobit2, x = seq(-0.9, 0.9, length.out = no.steps), xlim = c(-1, 1),
#      xlab = expression(rho), ylab = "Log-likelihood", type = "l")
# #For illustrative purposes, we include a line and legend representing the
# #original MLE for rho below, in red.
# abline(v = abs(MLE[2]), col = "red")
# legend(x = "bottomleft", legend = c("MLE"), col = c("red"), pch = 16)
# 
# #Think about this again.
# thing = Vectorize(function(par) ll.Tobit2.rho(par))
# curve(thing, c(-1, 1), n = 500)
# #NOTE: This seems really flat - is this actually correct?
# #nah.
```
