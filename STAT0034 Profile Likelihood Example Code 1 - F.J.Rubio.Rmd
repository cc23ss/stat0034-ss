...
prof.lik = function(parint, ind, init = "zero", max.iter = 1E4){
  ...
  #MODIFICATION - Add the max.iter variable in order to choose the number 
  #of nlminb iterations. 
...
prof1b = Vectorize(function(par) prof.lik(parint = log(par), ind = 1, 
                                          init = "MLE", max.iter = 1E5))

curve(prof1b, 1.2, 1.35, n = 50, lwd = 2, xlab = expression(sigma), 
      ylab = "Profile Likelihood")

#Same result - not a numerical/no. of iterations problem, perhaps it is that the
#profile likelihood of this particular example, e.g. parameters, have issues 
#here.
...
#For the next two we optimise and plot the profile likelihood like for prof1a. 
