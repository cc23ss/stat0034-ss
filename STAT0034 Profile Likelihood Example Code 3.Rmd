...
#h(t; beta_s, beta_o, xi) = h_0(t*exp{x^~'_i%*%beta_s}, eta)exp{x'_i%*%beta_o}})
#which defines the General Hazard (GH) structure. We optimise (OPT) over the GH
#structure, for the cases beta_s = 0 (Proportional hazards, PH), beta_s = beta_o, 
#x^~'_i = x'_i (Accelerated Failure Time, AFT), and the case where we have NO
#covariates at all, i.e. beta_s = beta_o = 0 (0). 
...
#Can also use the BC_a method from DiCiccio and Efron (1996). Possible to 
#estimate all required parameters. Should ask. 
...
BC_a.ci = function(B, MLE, ind, alpha, booted.MLEs){
  ...
  for (i in 1:n){
    ...
    OPT_i = GHMLE(init = c(rep(0, ncol(xt) + 3 + ncol(x))), times = times[-i], 
               status = status[-i], hstr = "GH", dist = "PGW", des = x[-i,], 
               des_t = xt[-i,], method = "nlminb", maxit = 1E4)
    #Getting the MLEs without i, as detailed above in the regular case. 
    MLE_i = c(exp(OPT_i$OPT$par[1:3]), OPT_i$OPT$par[-c(1:3)])
...
par1.CI.BC_a = BC_a.ci(B, MLE.PGWGH, 1, 0.05, MLE.B)
par1.CI.BC_a
par1.CI
par2.CI = quantile(MLE.B[, 2], c(0.025, 0.975))
par2.CI.BC_a = BC_a.ci(B, MLE.PGWGH, 2, 0.05, MLE.B)
par2.CI.BC_a
par2.CI
```
...
curve(prof4,-5,5, n = 500, lwd = 2, xlab = expression(beta_s[1]), ylab = "Profile Likelihood", ylim = c(0,1))
prof5 = Vectorize(function(par) prof.lik(par,5))
curve(prof5,-2,2 , n = 200, lwd = 2, xlab = expression(beta_o[1]), ylab = "Profile Likelihood", ylim = c(0,1))
prof6 = Vectorize(function(par) prof.lik(par,6))
curve(prof6,-1.1, -0.1 , n = 200, lwd = 2, xlab = expression(beta_o[2]), ylab = "Profile Likelihood")
prof7 = Vectorize(function(par) prof.lik(par,7))
curve(prof7,0,0.65, n = 200, lwd = 2, xlab = expression(beta_o[3]), ylab = "Profile Likelihood")
detach(lung)
